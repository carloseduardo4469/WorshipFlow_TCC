...
export default async function RegistrosEscalasPage() {
  ...
  return (
    <div className="mx-auto max-w-[1240px]">
      <div className="mb-7 flex flex-col items-start justify-between gap-2 sm:flex-row sm:items-center sm:gap-4"><PageHeader title="Registro de escalas" description="Crie, edite e acompanhe a programação das equipes." /><Link href="/dashboard/admin/escalas/novo" className="db-cta"><Plus size={16} />Nova escala</Link></div>
      <DataTable headers={["Título", "Data", "Status", "Equipe", ""]} isEmpty={escalas.length === 0} emptyMessage="Nenhuma escala cadastrada ainda.">
        {escalas.map((escala) => <tr key={escala.id}><td className="px-4 py-3.5 text-paper font-medium">{escala.titulo}</td><td className="px-4 py-3.5 text-muted">{formatDate(escala.dataEscala)}</td><td className="px-4 py-3.5"><StatusBadge status={escala.status} /></td><td className="px-4 py-3.5 text-muted">{escala.usuarioIds.length === 0 ? "—" : escala.usuarioIds.map((id) => nomesPorId.get(id) ?? "?").join(", ")}</td><td className="px-4 py-3.5 text-right"><div className="flex justify-end gap-3"><Link href={`/dashboard/admin/escalas/${escala.id}`} className="db-btn-sm">Editar</Link><form action={removerEscalaAction}><input type="hidden" name="id" value={escala.id} /><button type="submit" className="text-xs font-semibold text-red-400 hover:text-red-300">Excluir</button></form></div></td></tr>)}
      </DataTable>
    </div>
  );
}
