...
export default async function DashboardHomePage() {
  ...
  const completeProfile = Boolean(profile.instrumentoPrincipal && profile.habilidades);
  const overview = [
  ...
  return <div className="mx-auto max-w-[1240px]">
    <section className="db-hero relative overflow-hidden px-5 py-7 sm:px-8 sm:py-10 lg:px-10 lg:py-10"><div className="db-hero-light db-hero-light-left" /><div className="db-hero-light db-hero-light-right" />
      {!completeProfile && <Link href="/dashboard/perfil" className="db-profile-nudge relative z-10 ml-auto mb-8 flex max-w-[290px] items-center gap-3 lg:absolute lg:right-8 lg:top-0"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-[#ffea9a] to-[#c8962d] text-[#3a2a06]"><Check size={18} strokeWidth={3} /></span><span><strong className="block text-sm text-[#fff3b7]">Complete seu cadastro</strong><small className="mt-0.5 block text-xs font-medium text-[#c3cedb]">Abra o perfil e informe suas habilidades.</small></span><ArrowUpRight size={16} className="ml-auto text-[#f1d366]" /></Link>}
