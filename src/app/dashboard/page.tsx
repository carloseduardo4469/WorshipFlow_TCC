...
export default async function DashboardHomePage() {
  ...
  const [activeSchedules, musicasCount, repertoriosCount, teamCount] = await Promise.all([
    ...
    cachedData("repertorios:count", () => repos.repertorios.count()),
  ...
  const overview = [
    ...
    { label: "Favoritos", value: `${repertoriosCount} ${repertoriosCount === 1 ? "louvor" : "louvores"}`, text: "Repertório pessoal salvo para consulta rápida.", href: "/dashboard/repertorios", tint: "cyan" },
  ];
  return <div className="mx-auto max-w-[1240px]">
    ...
    <section className="mt-5 grid grid-cols-1 gap-5 xl:grid-cols-2"><DashboardPanel eyebrow="Mês" title={`Escalas de ${monthLabel()}`} href="/dashboard/escalas" empty={activeSchedules === 0 ? "Nenhuma escala prevista até o fim deste mês." : `${activeSchedules} escala(s) ativa(s) para acompanhar.`} icon={<CalendarDays size={18} />} /><DashboardPanel eyebrow="Repertório" title="Top louvores" href="/dashboard/repertorios" empty={musicasCount === 0 ? "Nenhum louvor foi usado em escalas ainda." : `${musicasCount} música(s) cadastrada(s) no repertório.`} icon={<ListMusic size={18} />} /></section>
  </div>;
}
