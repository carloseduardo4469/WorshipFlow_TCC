...
import { ArrowUp, ChevronDown, Pencil, Plus, Search, SlidersHorizontal, X } from "lucide-react";
    ...
    <div className="db-music-panel db-card p-3 sm:p-4">
      <div>
        ...
        <div className="relative min-w-0">
          {musicas.length === 0 ? <div className="db-empty db-empty-modern">Nenhuma música cadastrada ainda.</div> : <div ref={listaRef} className="db-music-scroll max-h-[min(68vh,720px)] overflow-y-auto overscroll-contain rounded-xl"><table className="db-table w-full text-left text-sm md:table"><thead className="sticky top-0 z-10 hidden bg-[#0b1b30] text-xs uppercase tracking-wide text-muted md:table-header-group"><tr><th className="px-4 py-3 font-medium">Título</th><th className="px-4 py-3 font-medium">Artista</th><th className="px-4 py-3 font-medium">Tom</th><th className="px-4 py-3 font-medium">BPM</th><th className="px-4 py-3 font-medium"><span className="sr-only">Ações</span></th></tr></thead><tbody className="grid gap-3 md:table-row-group md:divide-y md:divide-[color:rgba(148,163,184,0.1)]">{musicasFiltradas.map((musica) => <tr key={musica.id} className="grid grid-cols-2 gap-x-4 rounded-xl border border-white/[0.08] bg-white/[0.02] md:table-row md:rounded-none md:border-0 md:bg-transparent"><td className="col-span-2 px-4 pb-1 pt-3.5 text-paper font-medium md:table-cell md:py-3.5">{musica.linkCifra ? <a href={musica.linkCifra} target="_blank" rel="noreferrer" className="break-words hover:text-cyan-300 hover:underline">{musica.titulo}</a> : musica.titulo}</td><td className="px-4 py-1.5 text-muted md:table-cell md:py-3.5"><span className="mr-2 text-xs uppercase text-muted/70 md:hidden">Artista:</span>{musica.artista ?? "—"}</td><td className="px-4 py-1.5 font-mono text-amber md:table-cell md:py-3.5"><span className="mr-2 font-sans text-xs uppercase text-muted/70 md:hidden">Tom:</span>{musica.tonalidade ?? "—"}</td><td className="px-4 py-1.5 text-muted md:table-cell md:py-3.5"><span className="mr-2 text-xs uppercase text-muted/70 md:hidden">BPM:</span>{musica.bpm ?? "—"}</td><td className="col-span-2 px-4 pb-3 pt-1.5 text-right md:table-cell md:py-3.5">{isAdmin && <div className="db-row-actions"><button type="button" onClick={() => setMusicaAberta(musica)} className="db-btn-sm"><Pencil size={14} />Editar</button><form action={removerMusicaAction}><input type="hidden" name="id" value={musica.id} /><button type="submit" className="text-xs font-semibold text-red-400 hover:text-red-300">Excluir</button></form></div>}</td></tr>)}</tbody></table>{musicasFiltradas.length === 0 && <div className="db-empty px-4 py-10">Nenhuma música encontrada.</div>}</div>}
          {musicasFiltradas.length > 0 && <button type="button" onClick={() => listaRef.current?.scrollTo({ top: 0, behavior: "smooth" })} className="db-icon-button absolute bottom-3 right-3 z-20 h-10 w-10 shadow-lg" aria-label="Voltar ao topo da lista de músicas" title="Voltar ao topo"><ArrowUp size={18} /></button>}
        </div>
      </div>
    </div>
